...
export default function Home() {
  ...
  const loadConversations = async (uid: string) => {
    // Fetch sent messages and owned donation IDs in parallel
    const [{ data: sent }, { data: myDons }] = await Promise.all([
      supabase.from('messages').select('*').eq('sender_id', uid).order('created_at', { ascending: true }),
